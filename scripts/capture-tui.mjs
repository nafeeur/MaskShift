...
import { encodePng } from '../src/tui/image/png.mjs';

// Every capture below renders through the half-block fallback rather than
// whatever the host terminal running this script actually supports — a
// Kitty/iTerm2 placement is an opaque escape sequence with no pixels an SVG
// could show, but half-block is plain coloured text, which the ANSI parser
// below already knows how to turn into rectangles. Forcing it here is what
// makes the browser/image screenshots below possible at all, and it's also
// what keeps every capture identical regardless of whose terminal ran
// `npm run capture`.
process.env.MASKSHIFT_IMAGE = 'halfblock';
...
// --------------------------------------------------------------- demo images
//
// Both the Files preview and the BROWSER view show a decoded raster image,
// not text this script can just print — so unlike every other capture above,
// these two need real pixels to decode. Synthesized here (plain shapes over
// an RGBA buffer, encoded with the same encodePng the renderer itself uses
// to convert non-PNG formats) rather than shipped as a binary fixture, to
// keep the "zero dependencies, nothing that isn't generated by the product
// itself" rule intact for documentation too.

function setPixel(rgba, width, x, y, [r, g, b, a = 255]) {
  if (x < 0 || y < 0 || x >= width) return;
  const index = (y * width + x) * 4;
  if (index < 0 || index >= rgba.length) return;
  rgba[index] = r; rgba[index + 1] = g; rgba[index + 2] = b; rgba[index + 3] = a;
}

function fillRect(rgba, width, x0, y0, w, h, colour) {
  for (let y = y0; y < y0 + h; y += 1) for (let x = x0; x < x0 + w; x += 1) setPixel(rgba, width, x, y, colour);
}

function fillCircle(rgba, width, cx, cy, radius, colour) {
  for (let y = cy - radius; y <= cy + radius; y += 1) {
    for (let x = cx - radius; x <= cx + radius; x += 1) {
      if ((x - cx) ** 2 + (y - cy) ** 2 <= radius * radius) setPixel(rgba, width, x, y, colour);
    }
  }
}

/** A little mock landing page — nav bar, hero headline block, a call-to-
 *  action button and a row of content cards — standing in for whatever real
 *  site the live BROWSER view would actually be pointed at. */
function buildDemoPageFrame(width, height) {
  const rgba = Buffer.alloc(width * height * 4);
  for (let y = 0; y < height; y += 1) {
    const t = y / height;
    const top = [244, 247, 252]; const bottom = [225, 233, 245];
    const colour = [0, 1, 2].map((index) => Math.round(top[index] + (bottom[index] - top[index]) * t));
    fillRect(rgba, width, 0, y, width, 1, colour);
  }
  fillRect(rgba, width, 0, 0, width, Math.round(height * 0.11), [24, 28, 46]);
  fillRect(rgba, width, Math.round(width * 0.04), Math.round(height * 0.03), Math.round(width * 0.05), Math.round(height * 0.05), [64, 216, 197]);
  fillRect(rgba, width, Math.round(width * 0.7), Math.round(height * 0.04), Math.round(width * 0.08), Math.round(height * 0.035), [64, 216, 197]);
  fillRect(rgba, width, Math.round(width * 0.82), Math.round(height * 0.04), Math.round(width * 0.08), Math.round(height * 0.035), [148, 156, 184]);
  fillRect(rgba, width, Math.round(width * 0.22), Math.round(height * 0.22), Math.round(width * 0.56), Math.round(height * 0.09), [30, 38, 64]);
  fillRect(rgba, width, Math.round(width * 0.32), Math.round(height * 0.34), Math.round(width * 0.36), Math.round(height * 0.045), [120, 130, 156]);
  fillRect(rgba, width, Math.round(width * 0.38), Math.round(height * 0.44), Math.round(width * 0.24), Math.round(height * 0.075), [64, 216, 197]);
  const cardY = Math.round(height * 0.62); const cardHeight = Math.round(height * 0.28);
  const cardWidth = Math.round(width * 0.24); const gap = Math.round(width * 0.04);
  const accents = [[255, 182, 72], [92, 200, 255], [177, 132, 255]];
  for (let index = 0; index < 3; index += 1) {
    const x = Math.round(width * 0.08) + index * (cardWidth + gap);
    fillRect(rgba, width, x, cardY, cardWidth, cardHeight, [255, 255, 255]);
    fillRect(rgba, width, x, cardY, cardWidth, Math.round(cardHeight * 0.14), accents[index]);
  }
  return { width, height, rgba };
}

/** A small abstract sunset thumbnail — a generic "here's a picture" stand-in
 *  for the Files preview capture, distinct from the page mockup above so
 *  the two screenshots don't look like the same asset twice. */
function buildDemoPictureFrame(width, height) {
  const rgba = Buffer.alloc(width * height * 4);
  const horizon = Math.round(height * 0.62);
  for (let y = 0; y < horizon; y += 1) {
    const t = y / horizon;
    const top = [43, 25, 82]; const bottom = [255, 145, 96];
    const colour = [0, 1, 2].map((index) => Math.round(top[index] + (bottom[index] - top[index]) * t));
    fillRect(rgba, width, 0, y, width, 1, colour);
  }
  fillCircle(rgba, width, Math.round(width * 0.68), Math.round(horizon * 0.72), Math.round(height * 0.16), [255, 214, 120]);
  for (let y = horizon; y < height; y += 1) {
    const t = (y - horizon) / Math.max(1, height - horizon);
    const top = [64, 40, 92]; const bottom = [20, 14, 36];
    const colour = [0, 1, 2].map((index) => Math.round(top[index] + (bottom[index] - top[index]) * t));
    fillRect(rgba, width, 0, y, width, 1, colour);
  }
  return { width, height, rgba };
...
try {
  ...
  // These two decode a real raster image, so unlike every capture above they
  // need actual bytes on disk / in memory rather than just app state — see
  // "demo images" above. Appended last so overriding app.workspace.path for
  // the Files preview doesn't affect any capture before it.
  const demoPicture = buildDemoPictureFrame(120, 90);
  const demoPicturePath = path.join(home, 'sunset.png');
  await fsp.writeFile(demoPicturePath, encodePng(demoPicture.width, demoPicture.height, demoPicture.rgba));
  const realWorkspace = app.workspace;

  const demoPage = buildDemoPageFrame(1180, 760);
  const demoPageBuffer = encodePng(demoPage.width, demoPage.height, demoPage.rgba);

  captures.push(
    ['files-image', 'MaskShift — inline image preview', () => {
      app.workspace = { ...realWorkspace, path: home };
      app.previewPath = 'sunset.png';
      app.previewIsImage = true;
      app.previewError = '';
      app.previewLines = [];
      app._imagePreviewCache = null;
      app.fileList.selected = 0; // the tree itself is unrelated to sunset.png — don't highlight a mismatched row
      app.view = 'files'; app.focus = 'preview';
    }],
    ['browser', 'MaskShift — 07 BROWSER', () => {
      app.browserTarget = { instanceId: 'demo', tabId: 'demo' };
      app.browserFrame = {
        buffer: demoPageBuffer, cssWidth: demoPage.width, cssHeight: demoPage.height,
        title: 'Example — a page open in a real tab', url: 'https://example.test/', error: null,
      };
      app.browserFrameId = 1;
      app.view = 'browser'; app.focus = 'browser';
    }],
  );
